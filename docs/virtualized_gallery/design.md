...
- **タップで編集モード**: 中身が**正式な URI (`pixiv://default/user/12345` 等) に
  切り替わり全選択される**。そのままコピーすれば場所を持ち出せる (標準のテキスト
  選択 UI だけで済み、専用のコピー機能を作らない)
...
- `+` = 新しいタブを開く (現状のまま)
- `☰` = この画面・アプリの操作: 再読み込み、設定 (現在 `+` の中にいるが、
  「タブを開く」ではないのでこちらへ移す)、将来の履歴一覧など

#### 残る検討

- Pixiv のセクションメニュー (Pixiv ▾: トップ/ブックマーク/作者) の置き場。
  当面はボディのヘッダーに残し、ツールバーが安定してから再考する
...
4. **フェーズ 2C**: ナビゲーションツールバー (上記 7、[ADR 009](../adr/009-navigation-toolbar.md))。
   ...
   | 2C-3 | 検索オプション (完全/新着) を窓に取り込み、Pixiv ボディの検索ヘッダーを撤去 | 移設 |
