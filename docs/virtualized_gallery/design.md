...
- `N+` は「N 枚**以上**」(pageCount >= N)。旧実装は `>N` (超) だった。
  テキスト欄に `>N` と書いてあれば正しかったが、`3+` と書いたボタンでは嘘になる
- 表示するのは `hasPageCounts` が真のソースだけ (アイテムがページを持つか、
  という**データの事実**を dialect が答える)。将来 SMB の「動画のみ」等も同じ位置
- **値は `GallerySession` が持つ** (`anchor` の隣)。URI には入れない —
  入れるとフィルタを変えるたびに別の場所になり、既に持っている一覧を
  取り直すことになる。セッションが持つので**場所ごとに独立**で、戻ると
  そのときの絞り込みのまま戻る。変更は `onEntryChanged` → `tab.revision` で
  ヘッダーに、ボディには `didUpdateWidget` で伝わる (絞って画面が短くなったら
  次ページを読む必要があるため、再構築だけでは足りない)

#### スクロールでの自動隠し

2 段は常設できない: タブレット横画面で見えるサムネイルは 5 列 x 2.7 行しかない
(これが 2B-10 でストリップを AppBar に統合した理由でもある)。**2 段とも同じ高さ
44 で揃え** (`galleryHeaderRowHeight`)、下スクロールで両段とも隠れ、上スクロールで
戻す。隠れている間もシステム戻るは効くので、ナビ手段が消えるわけではない。

- **ヘッダーは `Scaffold.appBar` をやめてページの 1 行にした**。appBar の高さは
  Scaffold を組み立てた時点で決まるので、畳むアニメーションのたびにボディごと
  画面全体を作り直すことになる。Column の 1 行なら畳むのはヘッダーだけで済む
- `SliverAppBar(floating:)` は使えない。ヘッダーとグリッドが同じ
  `CustomScrollView` にいることが前提で、そうするとヘッダーがボディの中に入る。
  ヘッダーがホスト側にあるのはタブを跨いで状態 (ストリップのスクロール位置) を
  保つためなので、これは前提を壊す
- 判定は `ChromeScrollRule` (純粋)。**前進は 32px 貯まってから隠し、後退は即座に
  出す** — 戻す方に閾値を置くとヘッダーが渋く感じる。先頭では常に出す
- **リストが現れた時点で基準位置を教える** (`start`)。復元した場所は最初から
  深い位置に開くので、0 から測ると最初のひと振りで畳んでしまう。
  読み込み中はビューポートがまだ無いので、出来るまで待つ
- ステータスバーの余白はホスト側の `SafeArea` が持つ。ヘッダー側に持たせると、
  畳んだときにグリッドが時計の下へ潜り込む
