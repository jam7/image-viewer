...
#### ページ送りが遅いときのボトルネックは PNG エンコード

PDF が L2 にキャッシュ済みでもページ送りに数秒かかることがある。原因は SMB I/O
ではなく、`page.render()` (PDFium、scale 2.0) と `toByteData(format: png)` の
**PNG エンコード**。所要時間は PNG 出力サイズとほぼ線形に相関し、実測では
150〜230 KB のページが 0.2〜0.3 秒、4 MB 級のページが 5〜10 秒。エンコードは Dart
の UI アイソレートで走るため、その間ページ表示がブロックされる。

改善の方向 (いずれも未実施):

1. JPEG エンコードに変更 (品質 85 前後) — 漫画スキャンならサイズ 1/5〜1/10、
   エンコード自体も PNG より大幅に速い。効果大・実装小。透過や線画の劣化は要確認
2. 表示用はエンコードせず `ui.Image` のまま渡す — キャッシュ層 (L1/L2 は bytes
   前提) との兼ね合いで設計変更が必要
3. レンダリングスケールの動的調整 — 実寸 x scale 2.0 が画面解像度に対して過大な
   場合に落とす
