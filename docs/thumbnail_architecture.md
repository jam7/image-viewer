...
- `aid=no` — 音声を完全に切る。volume 0 では聞こえないだけでデコードも
  プロキシ経由のダウンロードも続いていた
- `hr-seek` — **コンテナで使い分ける** (capture ごとに設定)。MP4 系
  (mp4, m4v, mov) はキーフレームシーク (`no`): キーフレーム位置が索引 (stss) に
  載っていて信用でき、直前キーフレーム 1 枚のデコードで済む。既定の精密シークは
  3 秒地点まで全フレームをデコードするため 1 本 1.5〜2 秒かかっていた。
  **それ以外 (wmv, avi 等) は精密シーク (`yes`)**: ASF などのキーフレーム
  フラグは嘘をつくことがあり、キーフレームシークだと参照無しの delta frame を
  デコードして縞状のマクロブロックノイズがタイルに出る (実際に出た)。
  精密シークは目標まで通しでデコードするので、途中で絵が治る
