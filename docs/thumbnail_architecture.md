...
- **それ以外 (wmv, avi 等): シークせず先頭から走査**。ASF などのキーフレーム
  ...
  **3 秒以降で黒率 50% 以下の最初のフレーム**を採る (黒いタイトルカードや
  ウォーターマーク板で始まるファイルが多く、固定 3 秒はそこに乗っていた)。
  3 秒より手前は明るくても採らない — 1 秒からにしたら、薄暗いフレームが
  黒率の網をくぐって旧来より悪いタイルになった。
  内容 20 秒まで見て全部黒なら、一番黒くないフレームで妥協する
