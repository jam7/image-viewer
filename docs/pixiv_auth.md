...
_onUrlChanged で URL を監視:
  ...
  - www.pixiv.net      → _showWebView = false（ローディングに戻す）→ pop(true)
```
...
- ログイン→ギャラリー遷移中にホーム画面が露出する（TODO に記載）
  - ログイン画面 pop 後、API WebView の pixiv.net ロード完了まで
    ホーム画面が見えてしまう（Cookie 有効時も同様）
  - 解決策: ログイン画面が API WebView のロードも完了してから pop する
