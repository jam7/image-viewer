...
## ビューア
- [x] PageUp/PageDown で 10 ページ飛ばし、Home/End で先頭/末尾
- [x] ページサイドバー（右端、ホバー/ドラッグでページジャンプ）
- [x] 表示ページを最優先でレンダリング（先読みより先にロード）
- [x] PDF 先読み枚数を 2 に制限（PDFium シリアル処理のため）
- [x] PdfDocument キャッシュ（同じ PDF を開き直さない）
- [x] レンダリング済み PNG を L2 に保持（ビューア離脱時に削除しない）
- [ ] 高速スクロール時はサムネイル表示、停止後にフル解像度に差し替え
  - `/ajax/illust/{id}/pages` の `thumb_mini` URLを利用
  - サイドバーをドラッグ中はサムネイルのみ、離したらフル画像ロード
...
## ギャラリー
- [x] スクロールバー（SMB / Pixiv 両方、常時表示、ドラッグ可能）
- [x] SMB ギャラリーに Home/End キー対応
...
- [x] プロキシの SMB 接続断リトライ
- [ ] ビューアで動画に到達したら VideoPlayerScreen に遷移（現在は動画を除外）
